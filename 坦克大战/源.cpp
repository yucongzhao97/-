...
public:
	void drawBullet(int face, int x, int y) {
		switch (face)
		{
		case FACE_LEFT:Bx = x - 60; By = y; bullet_x = Bx / 60; bullet_y = By / 60; left();  break;
		case FACE_RIGHT:Bx = x + 60; By = y; bullet_x = Bx / 60; bullet_y = By / 60; right(); break;
		case FACE_UP:By = y - 60; Bx = x; bullet_x = Bx / 60; bullet_y = By / 60; up(); break;
...
private:
	...
	void left() {
		distance = 0;
		int k = bullet_x - 1;
		while (getItem(getLevel(), bullet_y, k) == 0) {
			k--;
			distance += 60;
		}
		k++;
		...
		else if (getItem(getLevel(), bullet_y, bullet_x - 1)>0)
		{
			num--;
			changeItem(getLevel(), bullet_y, bullet_x - 1);
		}
		else
		{
			for (int i = 0; i < distance; i++) {
				Bx -= 1; putimage(Bx, By, &Bullet[0]);
				Sleep(2);
			}
			changeItem(getLevel(), bullet_y, bullet_x - k);
			InitMap();
		}
	}
	void right() {
	...
	void up() {
		distance = 0;
		int k = bullet_y - 1;
		while (getItem(getLevel(), k, bullet_x) == 0) {
			k--;
			distance += 60;
		}
		k++;
		...
		else if (getItem(getLevel(), bullet_y - 1, bullet_x)>0)
		{
			num--;
			changeItem(getLevel(), bullet_y - 1, bullet_x);
		}
		else
		{
			for (int i = 0; i < distance; i++) {
				By -= 1; putimage(Bx, By, &Bullet[2]);
				Sleep(2);
			}
			changeItem(getLevel(), bullet_y - k, bullet_x);
			InitMap();
		}
	}
...
public:
	void controller(char c)
	{
		...
		case ' ':drawBullet(FACE_DOWN, x, y); break;
		default:break;
		}
		InitMap();
	}
